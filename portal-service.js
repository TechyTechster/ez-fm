#!/usr/bin/env node
/**
 * EZ File Manager - XDG Desktop Portal Service
 *
 * Implements the org.freedesktop.impl.portal.FileChooser interface
 * to allow EZ File Manager to act as the system-wide file picker.
 *
 * @license MIT
 */
...
function launchPicker(mode, multiple, currentName) {
  return new Promise((resolve) => {
    ...
    console.log(`[Portal] Launching: npx electron . ${args.join(' ')}`);

    const child = spawn('npx', ['electron', '.', '--ozone-platform=wayland', '--enable-features=UseOzonePlatform', ...args], {
      cwd: APP_PATH,
